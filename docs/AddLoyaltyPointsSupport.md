...
**support_request_id** | **int** | ID of the support request to approve. When provided by an admin, the points are added on behalf of the support user who created the request. | [optional] 
**processing_note** | **str** | Note from the admin approving the support request. Stored as the processing note on the support request record. This is only used when a supportRequestId is passed. | [optional] 
