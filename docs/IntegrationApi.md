...
> LoyaltyBalances get_loyalty_balances(loyalty_program_id, integration_id, end_date=end_date, subledger_id=subledger_id)
...
subledger_id = ['subledger_id_example'] # list[str] | Filter results by one or more subledger IDs. Must be exact match. (optional)
    ...
    try:
        # Get customer's loyalty points
        api_response = api_instance.get_loyalty_balances(loyalty_program_id, integration_id, end_date=end_date, subledger_id=subledger_id)
        pprint(api_response)
    except ApiException as e:
...
> LoyaltyBalances get_loyalty_card_balances(loyalty_program_id, loyalty_card_id, end_date=end_date, subledger_id=subledger_id)
